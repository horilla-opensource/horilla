...
from hydra_audit.methods import remove_duplicate_history
...
class HydraAuditInfo(models.Model):
    """
    HydraAuditInfo model to store additional info
    """
...
class HydraAuditLog(HistoricalRecords):
    """
    Model to store additional information for historical records.
    """

    # def __init__(self, *args, bases=None, **kwargs):
    #     super(HydraAuditLog, self).__init__(*args, **kwargs)
    #     self.is_hydra_audit_log = True
...
def pre_create_hydra_audit_log(sender, instance, *args, **kwargs):
    """
...
def post_create_hydra_audit_log(sender, instance, *_args, **kwargs):
    """
    ...
    try:
        ...
        if isinstance(history_instance, HydraAuditLog):
