...
def dashboard_attendance_trend(request):
    ...
    try:
        ...
        for i in range(11, -1, -1):
            ...
            weeks.append(
                {"week": week_start.strftime("%b %d"), "rate": rate, "present": present}
            )
    except Exception:
...
def dashboard_announcement_detail(request, pk):
    ...
    if employee:
        AnnouncementView.objects.get_or_create(
            announcement=ann, user=request.user, defaults={"employee": employee}
        )
...
def dashboard_pending_approvals(request):
    ...
    try:
        ...
        pending["payslips_review"] = payslip_count
    except Exception:
        pending["payslips_review"] = 0
