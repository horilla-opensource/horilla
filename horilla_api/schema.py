...
from drf_yasg.inspectors import SwaggerAutoSchema, FilterInspector
                ...
                if not should_cleanup:
                    try:
                        if hasattr(self.view, 'filterset_class') and self.view.filterset_class:
                            if hasattr(self.view.filterset_class, '_meta') and hasattr(self.view.filterset_class._meta, 'model'):
                                model = self.view.filterset_class._meta.model
                                self.view.queryset = model.objects.none()
                                should_cleanup = True
                            else:
                                return []
                        elif hasattr(self.view, 'serializer_class') and self.view.serializer_class:
                            if hasattr(self.view.serializer_class, 'Meta') and hasattr(self.view.serializer_class.Meta, 'model'):
