...
def faq_category_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, _('The FAQ Category created successfully.')
            )
...
def faq_category_update(request,id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.info(
                request, _('The FAQ category updated successfully.')
            )
            return HttpResponse("<script>window.location.reload()</script>")
    context = {
        'form': form,
...
def faq_category_delete(request, id):
    try:
        ...
        faq.delete()
        messages.success(
            request, _('The FAQ category has been deleted successfully.')
        )
...
def create_faq(request,cat_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, _('The FAQ created successfully.')
            )
            return HttpResponse("<script>window.location.reload()</script>")
    context = {
        'form': form,
...
def faq_update(request,id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.info(
                request, _('The FAQ updated successfully.')
            )
            return HttpResponse("<script>window.location.reload()</script>")
    context = {
        'form': form,
...
def faq_search(request):
    ...
    if query:
        results_list = SearchQuerySet().filter(Q(question__icontains=query) | Q(answer__icontains=query)).using("default")
...
def ticket_view(request):
    ...
    my_tickets = tickets.filter(is_active=True, employee_id = request.user.employee_get).order_by("-created_date")
    ...
    if hasattr(user, 'employee_work_info'):
        ...
        if department:
            tickets_items = ticket_list.filter(raised_on = department.id,assigning_type = "department" )
            allocated_tickets += tickets_items
        if job_position:
            tickets_items = ticket_list.filter(raised_on = job_position.id,assigning_type = "job_position" )
            allocated_tickets += tickets_items
...
def ticket_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():          
            ...
            mail_thread.start()
            messages.success(
                request, _('The Ticket created successfully.')
            )
...
def change_ticket_status(request,ticket_id):
    ...
    if ticket.status != status:
        if user == ticket.employee_id or user in ticket.assigned_to.all() or request.user.has_perm("helpdesk_changeticket"):
...
def get_allocated_tickets(request):
    ...
    tickets_items1 = Ticket.objects.filter(is_active=True,raised_on = department.id,assigning_type = "department" )
    # allocated_tickets += tickets_items
    tickets_items2 = Ticket.objects.filter(is_active=True,raised_on = job_position.id,assigning_type = "job_position" )
    # allocated_tickets += tickets_items
    tickets_items3 = Ticket.objects.filter(is_active=True,raised_on = user.id,assigning_type = "individual" )
    # allocated_tickets += tickets_items
...
def ticket_filter(request):
    ...
    if field != "" and field is not None:
        ...
        my_tickets = my_tickets.order_by(field_copy)
        all_tickets = all_tickets.order_by(field_copy)
        tickets_items1 = tickets_items1.order_by(field_copy)
        tickets_items2 = tickets_items2.order_by(field_copy)
        tickets_items3 = tickets_items3.order_by(field_copy)
...
def ticket_detail(request,ticket_id,**kwargs):
    ...
    for comment in comments :
        activity_list.append({"type" :"comment", "comment": comment,"date": comment.date})
    for history in trackings:
        activity_list.append({"type" :"history","history": history,"date": history['pair'][0].history_date})
...
def ticket_change_assignees(request,ticket_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_assignee_ids =form.cleaned_data['assigned_to'].values_list('id',flat=True)
        ...
        if c_form.is_valid():
            ...
            if request.FILES:
                ...
                if f_form.is_valid():
                    ...
                    for file in files:
                        a_form =AttachmentForm({"file":file,'comment':comment,"ticket":ticket})
                        a_form.save()       
    ...
    if page_number == "all":
        if table == "all":
            tickets_filter = TicketFilter(
                filters, queryset=Ticket.objects.all()
            )
        elif table == 'my':
            tickets_filter = TicketFilter(
            ...
            tickets_filter = TicketFilter(
                filters, queryset= allocated_tickets
            ) 
