...
var pdfUploadButton = function(context) {
  var ui = $.summernote.ui;
  return ui.button({
    contents: '<i class="note-icon-file" title="PDF (Supports small size only)" /> PDFs',
    tooltip: 'Embed PDF(s) as Cards + Modal',
    click: function () {
      ...
      input.onchange = function () {
        handlePDFUploadMultiple(this.files, context);
      };
      input.click();
    }
  }).render();
...
function handlePDFUploadMultiple(files, context) {
...
// Handle Upload
function handlePDFUpload(file, context) {
  const reader = new FileReader();
  reader.onload = function(e) {
    const base64 = e.target.result;
    const fileName = file.name;
    const uniqueId = `pdfModal_${Date.now()}`;
...
function noteProperties() {
  return {
    toolbar: [
      ...
      ['height', ['height']],
      ['table', ['table']],
      ['insert', ['link', 'picture', 'video', 'pdfUpload']],
