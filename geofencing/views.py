...
class GeoFencingSetupPutDeleteAPIView(APIView):
    ...
    def delete(self, request, pk):
        ...
        if request.user.is_superuser or company == location.company_id:
            location.delete()
            return Response({"message": "GeoFencing location deleted successfully"}, status=status.HTTP_200_OK)
        raise serializers.ValidationError("Access Denied..")
...
def geo_location_config(request):
    ...
    if request.method == "POST":
        try:
            form = GeoFencingSetupForm(request.POST, instance=get_company_location(request))
        except:
