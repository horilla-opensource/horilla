...
$(document).ready(function () {
  ...
  var $select = $('#id_employee_id').select2({
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $.each(key_results, function (key, value) {
          ...
          var options = $(`<option value=" ${value.id}">`).text(value.key_result);
          $('#id_employee_key_results_id').append(options).trigger('change')
        });

        if(reporting_manager){
          //  assigning the reporting manager of the employee
          var options = $(`<option value=" ${reporting_manager.id}" selected="selected">`).text(`${reporting_manager.employee_first_name}${reporting_manager.employee_last_name}`);
