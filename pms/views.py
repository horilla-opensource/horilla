...
from pms.models import EmployeeKeyResult, EmployeeObjective, Comment, Feedback, QuestionTemplate, Question, Answer, Period, QuestionOptions, KeyresultFeedback
...
from .forms import QuestionForm, ObjectiveForm, KeyResultForm, FeedbackForm, ObjectiveCommentForm, PeriodForm, QuestionTemplateForm
    ...
    if request.method == 'POST':
        ...
        # if the objective is for a individual
            ...
            if form_objectives.is_valid():
                ...
                notify.send(request.user.employee_get, recipient=objective.employee_id.employee_user_id, verb='You got an OKR!.',redirect=f'/pms/objective-detailed-view/{obj_id}',icon='list-circle')
        ...
        # if the objective is based on particular job position or department
            ...
            if form_objectives.is_valid():
                ...
                employee_work_informations = objective_type.employeeworkinformation_set.all()
                ...
                for employee in employee_work_informations:
                    ...
                    form.save()
                    notify.send(request.user.employee_get, recipient=form.employee_id.employee_user_id, verb='You got an OKR!.',redirect=f'/pms/objective-detailed-view/{form.id}',icon='list-circle')
    ...
    if request.method == 'POST':
        ...
        if objective_form.is_valid():
            ...
            if not instance.employee_id.id == int(request.POST.get('employee_id')):
                notify.send(request.user.employee_get, recipient=instance.employee_id.employee_user_id, verb='You got an OKR!.',redirect=f'/pms/objective-detailed-view/{instance.id}',icon='eye-outline')
            messages.info(request, _('Objective %(objective)s Updated') % {'objective': instance.objective})
...
def objective_delete(request, id):
    ...
    objective.delete()
    messages.success(request,_('Objective %(objective)s deleted') % {'objective':objective.objective})
...
def objecitve_filter_pagination(request, objective_own, objective_all):
    ...
    objective_filter_own = ObjectiveFilter(request.GET or initial_data, queryset=objective_own)
    objective_filter_all = ObjectiveFilter(request.GET or initial_data, queryset=objective_all)   
...
def objective_list_view(request):
    ...
    is_manager  = Employee.objects.filter(employee_work_info__reporting_manager_id=employee)
    
    if request.user.has_perm('pms.view_employeeobjective'):
        objective_own = EmployeeObjective.objects.filter(employee_id = employee).exclude(status='Closed')| EmployeeObjective.objects.filter(emp_obj_id__employee_id = employee).exclude(status='Closed')
        ...
        objective_own = EmployeeObjective.objects.filter(employee_id = employee).exclude(status='Closed') | EmployeeObjective.objects.filter(emp_obj_id__employee_id = employee).exclude(status='Closed') 
        objective_own = objective_own.distinct()
        objective_all = EmployeeObjective.objects.filter(employee_id__in=employees_ids).exclude(status='Closed') | EmployeeObjective.objects.filter(emp_obj_id__employee_id__in = employees_ids).exclude(status='Closed')
...
def objective_list_search(request):
    ...
    if request.user.has_perm('pms.view_employeeobjective'):
        # based on permission
        objective_own = EmployeeObjective.objects.filter(employee_id=request.user.employee_get).filter(employee_id__employee_first_name__icontains=objective_owner)
        objective_all = EmployeeObjective.objects.filter(employee_id__employee_first_name__icontains=objective_owner)
        context = objecitve_filter_pagination(request, objective_own, objective_all)

    elif is_manager:
        # if user is a manager
        employees_ids = [employee.id for employee in is_manager]
        objective_own = EmployeeObjective.objects.filter(employee_id=employee).filter(employee_id__employee_first_name__icontains=objective_owner)
        objective_all = EmployeeObjective.objects.filter(employee_id__in=employees_ids).filter(employee_id__employee_first_name__icontains=objective_owner)
        context = objecitve_filter_pagination(request, objective_own, objective_all)
...
def objective_detailed_view(request, emp_obj_id):
    ...
    completed_key_result = [obj.current_value for obj in key_results_all if obj.current_value == obj.target_value]
...
def objective_detailed_view_objective_status(request, id):
    ...
    objective.save()
    messages.info(request,_('Objective %(objective)s status updated') % {'objective':objective.objective})
    return redirect(objective_detailed_view_activity, id)
...
def objective_detailed_view_current_value(request, kr_id):
    ...
    if request.method == 'POST':
        ...
        if int(current_value) <= target_value:
            ...
            employee_key_result.save()
            messages.info(request,_('Current value of %(employee_key_result)s updated') % {'employee_key_result':employee_key_result})
...
def key_result_creation(request, obj_id, obj_type):
    ...
    if request.method == 'POST':
        if obj_type == 'individual':
            ...
            form_key_result = KeyResultForm(request.POST, initial={'employee_objective_id': employee_objective_id})
            ...
            for objective_id in objective_ids:
                ...
                form_key_result = KeyResultForm(request.POST, initial={'employee_objective_id': objective})
                if form_key_result.is_valid():
                    form = form_key_result.save(commit=False)
                    form.start_value = form.current_value
...
def key_result_creation_htmx(request, id):
    ...
    Returns:
        if errorr occure it will return errorr message .
    """
    context = {'key_result_form': KeyResultForm(),
               'objecitve_id': id}
    objective = EmployeeObjective.objects.get(id=id)
    if request.method == 'POST':
        ...
        if form_key_result.is_valid():
            ...
            form.save()
            messages.success(request,_("Key result created"))
            response = render(request,'okr/key_result/key_result_creation_htmx.html',context)
            return HttpResponse(response.content.decode('utf-8') +'<script>location.reload();</script>')
        context['key_result_form'] = form_key_result
...
def key_result_update(request, id):
    ...
    if request.method == 'POST':
        ...
        key_result_form.initial['employee_objective_id']= key_result.employee_objective_id  # adding intial objective value to the form
...
def send_feedback_notifications(request, form):
    ...
    if form.employee_id:
        ...
        notify.send(request.user.employee_get, recipient=employee.employee_user_id, verb='You have received feedback!', redirect=f'/pms/feedback-detailed-view/{form.id}', icon='chatbox-ellipses')

    # Send notification to manager
    if form.manager_id:
        manager = form.manager_id
        notify.send(request.user.employee_get, recipient=manager.employee_user_id, verb='You have been assigned as a manager in a feedback!', redirect=f'/pms/feedback-detailed-view/{form.id}', icon='chatbox-ellipses')

    # Send notification to subordinates
    if form.subordinate_id:
        subordinates = form.subordinate_id.all()
        for subordinate in subordinates:
            notify.send(request.user.employee_get, recipient=subordinate.employee_user_id, verb='You have been assigned as a subordinate in a feedback!', redirect=f'/pms/feedback-detailed-view/{form.id}', icon='chatbox-ellipses')

    # Send notification to colleagues
    if form.colleague_id:
        colleagues = form.colleague_id.all()
        for colleague in colleagues:
            notify.send(request.user.employee_get, recipient=colleague.employee_user_id, verb='You have been assigned as a colleague in a feedback!', redirect=f'/pms/feedback-detailed-view/{form.id}', icon='chatbox-ellipses')
...
def feedback_creation_ajax(request):
    ...
    if is_ajax:
        if request.method == 'POST':
            ...
            key_results = EmployeeKeyResult.objects.filter(employee_id=employee_id).values()
            employee_work_info = EmployeeWorkInformation.objects.filter(employee_id__id = employee_id).first()
            reporting_manager_id = employee_work_info.reporting_manager_id
            if reporting_manager_id:
                reporting_manager = {
                    "id": reporting_manager_id.id or None,
                    "employee_first_name": reporting_manager_id.employee_first_name or None,
                    "employee_last_name": reporting_manager_id.employee_last_name or None,
                 }
            else:
                 reporting_manager = None
            return JsonResponse({'key_results': list(key_results),'reporting_manager':reporting_manager})
...
@login_required
def filter_pagination_feedback(request, self_feedback, requested_feedback, all_feedback):
    """
    ...
    previous_data = request.environ['QUERY_STRING']
    initial_data = {'archive': False}  # set initial value of archive filter to False
    feedback_filter_own = FeedbackFilter(request.GET or initial_data, queryset=self_feedback)
    feedback_filter_requested = FeedbackFilter(request.GET or initial_data, queryset=requested_feedback)
    feedback_filter_all = FeedbackFilter(request.GET or initial_data, queryset=all_feedback)
...
def feedback_list_search(request):
    ...
    self_feedback = Feedback.objects.filter(employee_id=employee_id).filter(review_cycle__icontains=feedback)

    requested_feedback_ids = []
    requested_feedback_ids.extend([i.id for i in Feedback.objects.filter(manager_id=employee_id)])
    requested_feedback_ids.extend([i.id for i in Feedback.objects.filter(colleague_id=employee_id)])
    requested_feedback_ids.extend( [i.id for i in Feedback.objects.filter(subordinate_id=employee_id)])
    ...
    if request.user.has_perm('pms.view_feedback'):
        ...
        all_feedback = Feedback.objects.filter(
            employee_id__in=employees_id).filter(review_cycle__icontains=feedback)
        context = filter_pagination_feedback(
            request, self_feedback, requested_feedback, all_feedback)
    else:
...
def feedback_list_view(request):
    ...
    user = request.user
    employee = Employee.objects.filter(employee_user_id=user).first()
    feedback_requested_ids = Feedback.objects.filter(Q(manager_id=employee) | Q(colleague_id=employee) | Q(subordinate_id=employee)).values_list('id', flat=True)
    feedback_own = Feedback.objects.filter(employee_id=employee).exclude(status='Closed').filter(archive=False)
    feedback_requested = Feedback.objects.filter(pk__in=feedback_requested_ids).filter(archive=False)
    feedback_all = Feedback.objects.all().exclude(status='Closed').filter(archive=False)
    employees = Employee.objects.filter(employee_work_info__reporting_manager_id = employee) # checking the user is reporting manager or not 

    if request.user.has_perm('pms.view_feedback'):
        context = filter_pagination_feedback(request, feedback_own, feedback_requested, feedback_all)
    elif employees:
...
def feedback_answer_get(request, id):
    ...
    feedback_employees =  [feedback.employee_id] +[feedback.manager_id] + list(feedback.colleague_id.all()) + list(feedback.subordinate_id.all())
    ...
    for employee in feedback_employees:
        has_answer = Answer.objects.filter(employee_id=employee, feedback_id=feedback).exists()
    ...
    if request.method == 'POST':
        for question in questions:
            if request.POST.get(f'answer{question.id}'):
                ...
                Answer.objects.get_or_create(answer={'answer': answer}, question_id=question, feedback_id=feedback, employee_id=employee)
        ...
        for key_result in feedback.employee_key_results_id.all():
            if request.POST.get(f'key_result{key_result.id}'):
                ...
                KeyresultFeedback.objects.get_or_create(answer={'answer': answer}, key_result_id=key_result, feedback_id=feedback, employee_id=request.user.employee_get)
        messages.success(request,_('Feedback %(review_cycle)s has been answered successfully!.') % {'review_cycle':feedback.review_cycle})
        return redirect(feedback_list_view)
...
def feedback_answer_view(request, id):
    ...
    key_result_feedback = KeyresultFeedback.objects.filter(feedback_id=feedback, employee_id=employee)
...
def feedback_delete(request, id):
    ...
    if feedback.status == 'Closed' or feedback.status == 'Not Started' and not answered:
        feedback.delete()
        messages.success(request,_('Feedback %(review_cycle)s deleted successfully!') % {'review_cycle':feedback.review_cycle})
        return redirect(feedback_list_view)
    else:
        messages.warning(request,_('Feedback %(review_cycle)s is ongoing you  you can archive it!') % {'review_cycle':feedback.review_cycle})
...
def feedback_detailed_view_status(request, id):
    ...
    if (feedback.status) == status:
        messages.info(request,_('Feedback status updated to  %(status)s') % {'status':_(status)})
        return render(request,'messages.html')
    messages.info(request,_('Error occurred during status update to %(status)s') % {'status':_(status)})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if obj_question.question_type == '4':
                ...
                QuestionOptions(question_id=obj_question, option_a=option_a,
                                option_b=option_b, option_c=option_c, option_d=option_d).save()
...
def question_view(request, id):
    ...
    return render(request, 'feedback/question_template/question_template_detailed_view.html', context)
...
def question_update(request, temp_id, q_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, '\n'.join([f'{field}: {error}' for field, errors in form.errors.items() for error in errors]))
            return redirect(question_template_detailed_view, temp_id)
...
def question_delete(request, id):
    ...
    try:
        ...
        messages.error(request, _('Failed to delete question: Question template is in use.'))
        return redirect(question_template_detailed_view, temp_id)
...
def question_template_view(request):
    ...
    return render(request, 'feedback/question_template/question_template_view.html', context)
...
def question_template_update(request, id):
    ...
    return render(request, 'feedback/question_template/question_template_update.html', context)
...
def dashboard_view(request):
    ...
    if is_manager :
        employees_ids = [employee.id for employee in is_manager]
        count_objective = EmployeeObjective.objects.filter(employee_id__in=employees_ids).count()
        count_key_result = EmployeeObjective.objects.filter(emp_obj_id__employee_id__in = employees_ids).count()
        count_feedback = Feedback.objects.filter(employee_id__in =employees_ids).count()
        okr_at_risk = EmployeeObjective.objects.filter(employee_id__in=employees_ids).filter(status ='At Risk')
    else:
        count_objective = EmployeeObjective.objects.filter(employee_id = employee).count()
        count_key_result = EmployeeKeyResult.objects.filter(employee_id = employee).count()
        count_feedback = Feedback.objects.filter(employee_id = employee).count()
        okr_at_risk = EmployeeObjective.objects.filter(employee_id = employee).filter(status ='At Risk')
    context = {
...
def dashboard_objective_status(request):
    ...
    if is_ajax and request.method == 'GET':
        ...
        for status in objective_status:
            ...
            objectives_count = filtersubordinates(request,queryset=objectives,perm='pms.view_employeeobjective').count()
...
def dashboard_key_result_status(request):
    ...
    if is_ajax and request.method == 'GET':
        ...
        for i in key_result_status:
            ...
            key_results_count = filtersubordinates(request,queryset=key_results,perm='pms.view_employeekeyresult').count()
...
def dashboard_feedback_status(request):
    ...
    if is_ajax and request.method == 'GET':
        ...
        for i in feedback_status:
            ...
            feedback_count = filtersubordinates(request,queryset=feedbacks,perm='pms.view_feedback').count()
...
def filtersubordinates(request,queryset,perm=None):
    ...
    if manager:
        queryset =  queryset.filter(employee_id__employee_work_info__reporting_manager_id=manager) | queryset.filter(employee_id = manager)
        return queryset
    else:
