...
from base.models import Department, JobPosition, JobRole, WorkType, EmployeeShift, EmployeeType, Company
...
from base.methods import choosesubordinates, filtersubordinates, is_reportingmanager, filtersubordinatesemployeemodel, choosesubordinatesemployeemodel, sortby
from employee.forms import EmployeeForm, EmployeeBankDetailsForm, EmployeeWorkInformationForm, EmployeeProfileBankDetailsForm, UserForm, UserPermissionForm, EmployeeWorkInformationUpdateForm, EmployeeBankDetailsUpdateForm
...
def self_info_update(request):
    ...
    form = EmployeeForm(instance=Employee.objects.filter(
        employee_user_id=user).first())
    if request.POST:
        if request.POST.get('employee_first_name') is not None:
            instance = Employee.objects.filter(
                employee_user_id=request.user).first()
    ...
    return render(request, 'employee/profile/profile.html', {'form': form, 'bank_form': bank_form, })


@login_required
@manager_can_enter('employee.view_employee')
def employee_view_individual(request, id):
    """
    This method is used to view profile of an employee.
    """
    employee = Employee.objects.get(id=id)
    user = Employee.objects.filter(employee_user_id=request.user).first()
    if user.reporting_manager.filter(employee_id=employee).exists() or request.user.has_perm('employee.change_employee'):
        return render(request, 'employee/view/individual.html', {'employee': employee})
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/employee/employee-view'))
...
@require_http_methods(['POST'])
def employee_user_group_assign_delete(request, id):
    """
...
def employee_create(request):
    """
    This method will render employee creation form template and save if the form is valid
    """
    form = EmployeeForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            superuser_status = "superuser" in request.POST
            users = User.objects.all()
            username_exist = any(user.username == username for user in users)
            if not username_exist:
                form.save()
                messages.success(request, _('Employee added.'))
                new_user = User.objects.create_user(
                    username=username, email=username, password=password, is_superuser=superuser_status)
                new_user.groups.set(groups)
                new_user.user_permissions.set(permissions)
                new_employee = Employee.objects.get(email=username)
                new_employee.employee_user_id = new_user

                """
                default permissions
                """

                change_ownprofile = Permission.objects.get(
                    codename='change_ownprofile')
                view_ownworkinfo = Permission.objects.get(
                    codename='view_ownworkinfo')
                view_ownprofile = Permission.objects.get(
                    codename='view_ownprofile')
                new_user.user_permissions.add(view_ownprofile)
                new_user.user_permissions.add(change_ownprofile)
                new_user.user_permissions.add(view_ownworkinfo)
                new_employee.save()
    return render(request, 'employee_personal_info/employee_create_form.html', {'form': form, 'perm_form': permission_form})
...
def employee_view(request):
    ...
    filter = EmployeeFilter(queryset=employees)
    employees = filtersubordinatesemployeemodel(
        request, filter.qs, 'employee.view_employee')
    return render(request, 'employee_personal_info/employee_view.html', {'data': paginator_qry(employees, page_number), 'pd': previous_data, 'f': filter})
...
def employee_view_new(request):
    ...
    bank_form = EmployeeBankDetailsForm()
    f = EmployeeFilter(queryset=Employee.objects.all())
    return render(request, 'employee/create_form/form_view.html', {'form': form, 'work_form': work_form, 'bank_form': bank_form, 'f': f})


@login_required
@manager_can_enter('employee.change_employee')
def employee_view_update(request, id):
    """
    ...
    if user.reporting_manager.filter(employee_id=employee).exists() or request.user.has_perm('employee.change_employee'):
        ...
        if request.POST:
            if request.POST.get('employee_first_name') is not None:
                ...
                if work_form.is_valid():
                    ...
                    instance.employee_id = employee
                    instance.save()
                    notify.send(request.user.employee_get, recipient=instance.employee_id.employee_user_id,
                                verb='Your work details has been updated.', redirect='/employee/employee-profile', icon='briefcase')
                    messages.success(
                        request, _('Employee work information updated.'))
        ...
        return render(request, 'employee/update_form/form_view.html', {'form': form, 'work_form': work_form, 'bank_form': bank_form})
    return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/employee/employee-view'))
...
@permission_required('employee.change_employee')
def remove_profile_image(request, id):
    """
    This method is used to remove uploaded image
    args: id Employee model instance id
    """
    employee = Employee.objects.get(id=id)
...
@manager_can_enter('employee.change_employee')
@require_http_methods(['POST'])
def employee_update_personal_info(request, id=None):
    """
    ...
    if form.is_valid():
        ...
        if id is None:
            ...
            bank_form = EmployeeBankDetailsForm(
                instance=EmployeeBankDetails.objects.filter(employee_id=employee).first())
            return redirect(f'employee-view-update/{form.instance.id}/', data={'form': form, 'work_form': work_form, 'bank_form': bank_form})
        return HttpResponse(
            """
    ...
    if id is None:
        return render(request, 'employee/create_form/form_view.html', {'form': form, })
    errors = '\n'.join([
...
@require_http_methods(['POST'])
def employee_update_work_info(request, id=None):
    """
    This method is used to update employee work info
    """
    employee = Employee.objects.filter(id=id).first()
    form = EmployeeWorkInformationForm(
        request.POST, instance=EmployeeWorkInformation.objects.filter(employee_id=employee).first())
...
@require_http_methods(['POST'])
def employee_update_bank_details(request, id=None):
    """
    This method is used to render form to create employee's bank information.
    """
    employee = Employee.objects.filter(id=id).first()
    form = EmployeeBankDetailsForm(
        request.POST, instance=EmployeeBankDetails.objects.filter(employee_id=employee).first())
    if form.is_valid() and employee is not None:
        bank_info = form.save(commit=False)
        bank_info.employee_id = employee
        bank_info.save()
...
def employee_filter_view(request):
    ...
    filter = EmployeeFilter(request.GET, queryset=employees)
    page_number = request.GET.get('page')
    template = 'employee_personal_info/employee_card.html'
    view = request.GET.get('view')
    if view == 'list':
        template = 'employee_personal_info/employee_list.html'
    return render(request, template, {'data': paginator_qry(filter.qs, page_number), 'f': filter, 'pd': previous_data})
...
def employee_card(request):
    ...
    if type(search) is type(None):
        search = ''
    employees = filtersubordinatesemployeemodel(
        request, Employee.objects.all(), 'employee.view_employee')
    if request.GET.get('is_active') is None:
        filter = EmployeeFilter(request.GET, queryset=employees.filter(
            employee_first_name__icontains=search, is_active=True))
    else:
        filter = EmployeeFilter(request.GET, queryset=employees.filter(
            employee_first_name__icontains=search))
    page_number = request.GET.get('page')
    employees = sortby(request, filter.qs, 'orderby')
    return render(request, 'employee_personal_info/employee_card.html', {'data': paginator_qry(employees, page_number), 'f': filter, 'pd': previous_data})
...
def employee_list(request):
    ...
    if type(search) is type(None):
        search = ''
    if request.GET.get('is_active') is None:
        filter = EmployeeFilter(request.GET, queryset=Employee.objects.filter(
            employee_first_name__icontains=search, is_active=True))
    else:
        filter = EmployeeFilter(request.GET, queryset=Employee.objects.filter(
            employee_first_name__icontains=search))
    ...
    page_number = request.GET.get('page')
    return render(request, 'employee_personal_info/employee_list.html', {'data': paginator_qry(employees, page_number), 'f': filter, 'pd': previous_data})
...
def employee_update(request, id):
    ...
    work_info = EmployeeWorkInformation.objects.filter(
        employee_id=employee).first()
    bank_info = EmployeeBankDetails.objects.filter(
        employee_id=employee).first()
    work_form = EmployeeWorkInformationForm()
    bank_form = EmployeeBankDetailsUpdateForm()
    ...
    return render(request, 'employee_personal_info/employee_update_form.html', {'form': form, 'work_form': work_form, 'bank_form': bank_form})
...
def employee_bulk_delete(request):
    ...
    for id in ids:
        ...
        try:
            ...
            messages.error(request, e)
            messages.error(request, _('You cannot delete %(employee)s.') % {'employee':employee})
...
@login_required
@permission_required('employee.delete_employee')
def employee_archive(request, id):
    employee = Employee.objects.get(id=id)
...
@login_required
@permission_required('employee.change_employee')
def archive_employee(request, id):
    """
    This method is used to archive employee
    """
    employee = Employee.objects.get(id=id)
    employee.is_active = False
    messages.success(request, _('Employee archived successfully.'))
    return
...
def employee_search(request):
    ...
    employees = sortby(request, employees, 'orderby')
    return render(request, template, {'data': paginator_qry(employees, page_number), 'pd': previous_data})


@login_required
@manager_can_enter('employee.add_employeeworkinformation')
@require_http_methods(['POST'])
def employee_work_info_view_create(request, id):
    """
    This method is used to create employee work information from employee single view template
    args:
        id : employee instance id
    """
...
@manager_can_enter('employee.change_employeeworkinformation')
@require_http_methods(['POST'])
def employee_work_info_view_update(request, id):
    """
    This method is used to update employee work information from single view template
    args:
        id  : employee work information id
    """

    work_information = EmployeeWorkInformation.objects.get(id=id)
...
@require_http_methods(['POST'])
def employee_bank_details_view_create(request, id):
    """
...
@manager_can_enter('employee.change_employeebankdetails')
@require_http_methods(['POST'])
def employee_bank_details_view_update(request, id):
    """
    This method is used to update employee bank details.
    """
    employee_bank_instance = EmployeeBankDetails.objects.get(id=id)
...
def employee_work_information_delete(request, id):
    """
    This method is used to delete employee work information
    args:
        id : employee work information id
    """
    work_information = EmployeeWorkInformation.objects.get(id=id).delete()
...
def employee_import(request):
            ...
            try:
                ...
                if existing_user is None:
                    ...
                    if ' ' in employee_full_name:
                        employee_first_name, employee_last_name = employee_full_name.split(
                            ' ', 1)

                    user = User.objects.create_user(
                        username=email, email=email, password=str(phone).strip(), is_superuser=False)
                    employee = Employee()
    ...
    df = pd.DataFrame(columns=['employee_full_name', 'email', 'phone'])
...
def employee_export(request):
    ...
    df = pd.DataFrame(list(employee_data), columns=field_names)
...
def work_info_import(request):
    ...
    if request.method == 'POST' and request.FILES.get('file') is not None:
        file = request.FILES['file']
        df = pd.read_excel(file)
        work_info_dicts = df.to_dict('records')
        error_lists = []
            ...
            try:
                ...
                if user is None:
                    ...
                    employee_obj = Employee.objects.filter(
                        employee_first_name=first_name, employee_last_name=last_name).first()
                    ...
                    job_role_obj = JobRole.objects.filter(
                        job_role=job_role, job_position_id=job_position_obj).first()
                    ...
                    work_type_obj = WorkType.objects.filter(
                    ...
                    if type(reporting_manager) is str and ' ' in reporting_manager:
                        manager_fname, manager_lname = reporting_manager.split(
                            ' ', 1)
                    reporting_manager_obj = Employee.objects.filter(
                        employee_first_name=manager_fname, employee_last_name=manager_lname).first()
                    company_obj = Company.objects.filter(
                        company=company).first()
                    employee_work_info = EmployeeWorkInformation.objects.filter(
                        employee_id=employee_obj).first()
        ...
        if error_lists:
            ...
            # df = pd.DataFrame(res)
            df = pd.DataFrame(error_lists, columns=error_lists[0].keys())
...
def work_info_export(request):
    ...
    field_names += [f.name for f in EmployeeWorkInformation._meta.get_fields()
                    if not f.auto_created]

    fields_to_remove = ['badge_id', 'employee_id', 'job_position_id', 'department_id', 'work_type_id',
                        'job_role_id', 'reporting_manager_id', 'company_id', 'shift_id', 'employee_type_id', 'additional_info']
    field_names = [
    ...
    df = pd.DataFrame(employee_data, columns=field_names)
...
def get_employees_birthday(request):
    ...
    for emp in employees:
        ...
        try:
            ...
            path = f'https://ui-avatars.com/api/?name={emp.employee_first_name}+{emp.employee_last_name}&background=random'
        birthdays.append({
            'profile': path,
            'name': name,
            'dob': dob,
...
def dashboard(request):
    ...
    inactive_ratio = 0
    if employees.exists():
        active_ratio = "%.1f" % ((len(active_employees)/len(employees))*100)
        inactive_ratio = "%.1f" % (
            (len(inactive_employees)/len(employees))*100)

    return render(request, 'employee/dashboard/dashboard_employee.html', {
...
def dashboard_employee(request):
    ...
    """
    labels = ['Active', 'In-Avtive',]
    employees = Employee.objects.all()
    employees = filtersubordinates(
        request, employees, 'employee.view_employee')
    response = {'dataSet': [{'label': 'Employees', 'data': [len(employees.filter(
        is_active=True)), len(employees.filter(is_active=False)),]}, ], 'labels': labels}
...
def dashboard_employee_gender(request):
    ...
    response = {'dataSet': [{'label': 'Employees', 'data': [len(employees.filter(gender='male')), len(
        employees.filter(gender='female')), len(employees.filter(gender='others')),]},], 'labels': labels}
    return JsonResponse(response)
...
def dashboard_employee_department(request):
    ...
    for dept in departments:
        count.append(len(Employee.objects.filter(
            employee_work_info__department_id__department=dept)))
    response = {'dataSet': [
        {'label': 'Department', 'data': count}], 'labels': labels}
    return JsonResponse(response)
...
def dashboard_employee_tiles(request):
    ...
    data['newbies_today'] = EmployeeWorkInformation.objects.filter(
        date_joining__range=[date.today(), date.today() + timedelta(days=1)]).count()
    try:
        data['newbies_today_percentage'] = "{:.2f}%".format(EmployeeWorkInformation.objects.filter(date_joining__range=[date.today(
        ), date.today() + timedelta(days=1)]).count()/Employee.objects.filter(employee_work_info__isnull=False).count())
    except:
        data['newbies_today_percentage'] = 0
    # filtering newbies on this week

    data['newbies_week'] = EmployeeWorkInformation.objects.filter(
        date_joining__range=[date.today()-timedelta(days=7), date.today()]).count()
    try:
        data['newbies_week_percentage'] = "{:.2f}%".format(EmployeeWorkInformation.objects.filter(date_joining__range=[date.today(
        )-timedelta(days=7), date.today()]).count()/Employee.objects.filter(employee_work_info__isnull=False).count())
    except Exception as e:
