...
|---|---|---|
| 001 Horilla audit | Implemented | `HORILLA_AUDIT.md`, `REPOSITORY_MAP.md`, `REUSE_MATRIX.md` and the pinned upstream revision document reuse and upgrade risks. |
...
| 005 organization hierarchy | Implemented | Horilla Company/Department are reused; Location, Section, Team and effective assignments/scopes are implemented in `hydra_coordination`. |
...
| 007 Person identity | Implemented by EXTEND/WRAP | `hydra_people.Person` is canonical and links many Horilla Candidate applications and at most one Employee. Separate duplicate CandidateProfile/EmployeeProfile tables were intentionally not created. |
...
| 009 vacancies/applications | Implemented by EXTEND | Horilla Recruitment/Stage/Candidate are reused and wrapped by scoped PersonApplication services and selectors. |
| 010 recruitment workflow | Implemented by REUSE/EXTEND/WRAP | Horilla keeps Recruitment/Stage/Candidate and its pipeline UI. Linked applications now use one locked, scoped transition service, configurable directed requirements, authorized reasoned override and append-only history; direct save/bulk bypasses and the main legacy pipeline mutation routes are closed. |
...
| 013 candidate conversion | Implemented | One idempotent locked service creates/links Horilla Employee without creating a second Person and preserves conversion decisions. |
...
| 017 universal tasks | Implemented by NEW + REUSE/WRAP | `hydra_tasks` provides one scoped, idempotent and version-locked task linked to Person and an approved domain target, with append-only events, durable privacy-safe assignment notifications, Person timeline/UI and overdue coordinator integration. Horilla notifications are wrapped; its incompatible project/onboarding/helpdesk task ownership is not duplicated. |
| 018 notifications | Implemented by REUSE/WRAP | `hydra_notifications` wraps Horilla in-app rows with current-recipient target scope, fixed PII-free kinds, a paginated responsive center, versioned append-only read/archive state, opt-in generic email with durable leases/retry/dead-letter evidence, opt-in browser sound and an explicit no-native-push policy. Legacy rows are backfilled/wrapped and their mutation endpoints are POST-only and recipient-scoped. |
...
| 023 public onboarding portal | Partial | Stable external multilingual portal links are preserved and Horilla's token portal is wrapped. The package's later Django-hosted multilingual public location content is not implemented. |
| 024 onboarding content model | Implemented by EXTEND/WRAP + NEW MODULE | Horilla stages/tasks, token portal and controlled handoff remain authoritative. `hydra_onboarding` adds Company/language-scoped immutable published CourseVersion/Lesson/Quiz content, SHA-256 payload fingerprints, exact assignment snapshots, append-only attempts/confirmations/events, scoped UI, Person timeline and readiness integrity. |
...
| 027 employee assignments | Implemented | Effective-dated Team/Department history, locked reassignment, Horilla work-information projection and onboarding reconciliation are implemented. |
| 028 daily presence | Missing | The brigadier panel reads Horilla attendance/leave exceptions, but Hydra has no expected roster, exception write workflow or reasoned daily confirmation fact. |
