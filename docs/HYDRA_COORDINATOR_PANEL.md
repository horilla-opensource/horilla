...
The selected operational day cannot be in the future. The dashboard composes three independently scoped exception lists:
...
3. **Legalization attention**: cases for Persons with an active primary assignment in the Location, covering missing or overdue workflow deadlines, deadlines within 30 days, missing approved validity, validity ending within 30 days and expired validity.

The four summary counters show arrivals scheduled on the day and the full count of each exception group. Each detailed list is capped at 25 rows to keep the operational page bounded and mobile-friendly.
...
The browser journey was completed against the real PostgreSQL schema using the `hydra-qa` operator. At 390 x 844 pixels it exposed only direct-granted Browser Location A despite broader Company and Team grants, rendered KPI values `1 / 1 / 0 / 1`, an overdue arrival and an overdue legalization deadline, and kept the active Coordinator navigation state. The document width was 380 pixels for the 390-pixel viewport and the tested page emitted no console warnings or errors. Direct navigation to `?location=2` returned 404.
...
- Housing is not shown because no accepted Hydra housing implementation exists yet.
