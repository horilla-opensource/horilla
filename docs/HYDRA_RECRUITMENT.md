...
- creation of a standard Horilla Candidate from a scoped Person;
- automatic use of Horilla's active `initial` Stage;
...
- Horilla's `(email, recruitment)` uniqueness remains in force and is checked case-insensitively before save.
...
- continued operation of the original Horilla Candidate list;
...
4. Confirm the created record appears both in Hydra and in the original Horilla Candidate view with the automatic initial stage.
...
- Candidate/private document delivery remains unsafe in upstream Horilla. The completed `hydra_documents` boundary now stores sensitive Hydra files separately and does not use that path.
