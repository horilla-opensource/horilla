...
def organization(request):
    ...
    grants = ScopeGrant.objects.none()
    if request.user.has_perm("hydra_coordination.view_scopegrant"):
        grants = ScopeGrant.objects.select_related(
            "user", "company", "department", "location", "section", "team"
        )
        if not request.user.is_superuser:
            grants = grants.filter(user=request.user)
    return render(
        request,
        "hydra_coordination/organization.html",
        {"locations": locations, "scope_grants": grants[:50]},
    )
